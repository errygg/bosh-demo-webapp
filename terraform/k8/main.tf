...
resource "azurerm_resource_group" "consul-k8" {
  name     = "${var.namespace}-consul-k8"
...
resource "azurerm_virtual_machine" "consul_server" {
  ...
  provisioner "file" {
    ...
    destination = "${var.config_destination_dir}/license.lic"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep 5",
      "consul license put -http-addr=http://${azurerm_public_ip.consul_server.ip_address}:8500 @${var.config_destination_dir}/license.lic",
      "consul operator area create -http-addr=http://${azurerm_public_ip.consul_server.ip_address}:8500 -peer-datacenter=${var.peer_datacenter}",
      "consul operator area join -http-addr=http://${azurerm_public_ip.consul_server.ip_address}:8500 -peer-datacenter=${var.peer_datacenter} ${var.peer_datacenter_address}",
      "sleep 1"
    ]
...
resource "azurerm_virtual_machine" "consul_client" {
  ...
  provisioner "file" {
    ...
    content     = "${data.template_file.consul_template.rendered}"
  ...
  #"consul-template -consul-addr=http://${azurerm_public_ip.consul_server.ip_address}:8500 -template=${var.service_type}.sh.tpl:${var.service_type}.sh -once",
  #"nohup ${var.config_destination_dir}/${var.service_type}.sh > ${var.config_destination_dir}/${var.service_type}.log 2>&1 &",
  # Start the consul agent service and the dashboard/counter service
  provisioner "remote-exec" {
    inline = [
      "chmod 755 ${var.config_destination_dir}/init.sh",
      "sudo ${var.config_destination_dir}/init.sh",
...
data "template_file" "init_client" {
  ...
  vars     = {
    ...
    binary_filename    = "${var.consul_binary_filename}"
    binary_url         = "${var.consul_binary_url}"
    ct_binary_filename = "${var.ct_binary_filename}"
    ct_binary_url      = "${var.ct_binary_url}"
    datacenter         = "${var.datacenter}"
    data_directory     = "${var.data_directory}"
    log_level          = "${var.log_level}"
